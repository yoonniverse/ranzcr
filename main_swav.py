...
parser.add_argument('--last_conv', type=int)
parser.add_argument('--centercrop', type=float, default=1.)
parser.add_argument('--pos_emb', type=int)
...
def main():
    ...
    # build model
    model = SWAVWrapper(CFG(vars(args)))
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # train the network
        scores, queue = train(train_loader, model, optimizer, epoch, lr_schedule, queue, amp_scaler)
        ...
        if args.rank == 0:
            ...
            torch.save({'model': model.module.root.state_dict()}, os.path.join(args.dump_path, 'root_state_dict.pth'))
...
def train(train_loader, model, optimizer, epoch, lr_schedule, queue, amp_scaler):
